...
def parse_args(parser):
    ...
    parser.add_argument('--retrieve-num', action="store", dest='retrieve_num', help='retrieve number, default: 10 ', type=int, default=10)
...
def all_test(triplet,all_info,word_vector) :
    all_entity   = all_info[0]
    all_relation = all_info[1]
    idx2name     = all_info[2]
    name2idx     = all_info[3]
    ...
    for entity_l in all_entity :
        for relation in all_relation :
            for entity_r in all_entity :
                if name2idx[entity_l] == triplet[0] and name2idx[relation] == triplet[1] and name2idx[entity_r] == triplet[2] :
                    answer = count
                count = count + 1
                test_triplet = [name2idx[entity_l],name2idx[relation],name2idx[entity_r]]
                test_vector  = label2vector(test_triplet,all_info,word_vector)
                result = combine(result,test_vector)  
...
def label_sim(triplet_1,triplet_2) :
    count = 0
    for i in range(3) :
        if triplet_1[i] == triplet_2[i] :
            count = count + 1
    return count
