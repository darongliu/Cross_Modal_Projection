...
def one_rank(label_info , probability , answer ) :
    label_idx = label_info[answer]
    answer_p = probability[label_idx]
    rank = 1
    ...
    for key in label_info.keys() :
        idx = label_info[key]
        if probability[idx] >= answer_p :
            bigger_label.append(key)

    return rank , bigger_label

def unexist_tri_num(label_info , bigger_label_left , bigger_label_relation , bigger_label_right) :
    invalid = 0
    for left in bigger_label_left :
        if left not in label_info :
            invalid = invalid + len(bigger_label_relation)*len(bigger_label_right)
            continue
        else :
            for relation in bigger_label_relation :
                if relation not in label_info[left] :
                    invalid = invalid + len(bigger_label_right)
                    continue
                else :
                    for right in bigger_label_right :
                        if right not in label_info[left][relation] :
                            invalid = invalid + 1
                        else :
                            pass
    return invalid
