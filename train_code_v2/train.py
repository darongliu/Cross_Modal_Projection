...
#    print 'minibatch size: %d' % args.minibatch_size
    ...
    P , train, valid , _ , test = make_train(
...
def visualize(args) :
    v_data  = pickle.load(open(args.visualize[0],'rb'))
    v_label = pickle.load(open(args.visualize[1],'rb'))
    ...
    print '-------------------------------------------------------'
    print 'visualize data: %s'  % args.visualize[0]
    print 'visualize label: %s' % args.visualize[1]
    print 'load file: %s'       % args.load_net
    print '-------------------------------------------------------'


    temp = image_project(v_data[0])
    total_projection_vector = temp.reshape([1,temp.shape[0]])
    total_label = [v_label[0][0]]
    for image_vector , label in zip(v_data[1:],v_label[1:]) : 
        if image_vector == None :
            continue
        temp = image_project(image_vector)
        projection_vector = temp.reshape([1,temp.shape[0]])
        total_projection_vector = np.concatenate((total_projection_vector, projection_vector), axis=0)
        total_label.append(label[0])
    V.data_visualize(total_projection_vector,total_label,all_info)
